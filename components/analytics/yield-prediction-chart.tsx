...
export function YieldPredictionChart({ cropType }: YieldPredictionChartProps) {
      ...
      try {
        ...
        for (let year = currentYear + 1; year <= currentYear + 2; year++) {
          ...
          mockData.push({
            ...
            actual: null,
            predicted: predicted,
            lowerBound: Math.round((predicted - 1 - Math.random() * 0.5) * 10) / 10,
            upperBound: Math.round((predicted + 1 + Math.random() * 0.5) * 10) / 10,
          })
        }
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Impossible de charger les données de prédiction de rendement. Veuillez réessayer.
          </p>
        </CardContent>
      </Card>
    )
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Prédiction de rendement</CardTitle>
        <CardDescription>Analyse et prévision des rendements pour {cropName}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis dataKey="year" />
              <YAxis domain={[0, "dataMax + 2"]} label={{ value: "t/ha", angle: -90, position: "insideLeft" }} />
